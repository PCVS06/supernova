...
const {pi: artwork} = (await Bun.file(new URL("../../../packages/web/assets/constant-orbs.json", import.meta.url)).json()) as {
  pi: {
    readonly size: number;
    readonly glyphPath: string;
    readonly layers: readonly {readonly bands: readonly {readonly path: string; readonly opacity: number}[]}[];
  };
};
const pixels = artwork.layers.flatMap((layer) => layer.bands.map((band) => `<path d="${band.path}" opacity="${band.opacity}"/>`)).join("");
...
const tile = `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 ${artwork.size} ${artwork.size}"><rect width="${artwork.size}" height="${artwork.size}" fill="#000000"/>${drawing}</svg>`;

...
vi.mock("@tanstack/react-router", () => ({
  Link: (props: {children: ReactNode; className?: string; params?: Record<string, string>; to: string}) => (
    <a className={props.className} href={Object.entries(props.params ?? {}).reduce((current, [key, value]) => current.replace(`$${key}`, value), props.to)}>
      {props.children}
    </a>
  ),
}));
...
const render = (config?: HarnessConfig["curator"]) => renderToStaticMarkup(<CuratorEditor harness={{...harness, curator: config}} onChangeHarness={vi.fn()} />);
...
describe("curator configuration", () => {
  ...
  it("orders the panel from status through model, auto-apply, schedule and signals to the limits and requests", () => {
    const html = render();
    ...
    expect(html.indexOf(">Spend limits<")).toBeLessThan(html.indexOf(">Requests<"));
    ...
    // The review history moved to the inbox, which the page only links to.
  ...
  it("reports the last review, its spend and the way into the inbox", () => {
    ...
    expect(html).toContain('href="/inbox/science"');
    // Only the last review is reported here; the older ones are listed in the inbox.
    expect(html).not.toContain("The provider refused the request.");
  });

  it("offers the inbox even when no proposal waits", () => {
    const html = render();

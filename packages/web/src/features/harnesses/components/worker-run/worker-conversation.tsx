  ...
  return (
    <MathResponseContext value={{constant, revealedTurns}}>
      <section aria-label="Worker conversation" className="space-y-2">
        {result ? (
          <Disclosure key={run.id} defaultOpen label={active ? "Latest response" : "Result"}>
            <article aria-label={active ? "Latest response" : "Agent result"}>

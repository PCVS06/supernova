...
export default function ExpandableControlRow(props: ExpandableControlRowProps) {
  const {label, status, leading, actions, children, summary, expanded: controlledExpanded, onExpandedChange} = props;
  ...
  return (
    <div className="px-3 py-2">
      <div className="flex min-w-0 items-center gap-2">
        ...
        >
          ...
          <span className="grid size-8 shrink-0 place-items-center">
            <Icon
              className={expanded ? "rotate-90 transition-transform motion-reduce:transition-none" : "transition-transform motion-reduce:transition-none"}
              name="chevron-right"
              size="xs"
            />
          </span>
      ...
      {summary && <div className="pl-7 text-xs text-ink-muted">{summary}</div>}
      <AnimatedHeight>
